...
@Log4j
public class ChouetteTenantIdentifierGenerator implements IdentifierGenerator, Configurable {
	...
	private static List<ChouetteTenantIdentifierGenerator> instances = new ArrayList<>();

	private Map<String, State> states = new ConcurrentHashMap<>();

	public static void deleteTenant(String tenantIdentifier) {
		for (ChouetteTenantIdentifierGenerator instance : instances) {
			instance.states.remove(tenantIdentifier);
		}
	}

	public ChouetteTenantIdentifierGenerator() {
		instances.add(this);
	}
	...
	public void configure(Type type, Properties params, Dialect dialect) throws MappingException {
		...
		log.debug("----------------configure sequence " + sequenceName + " ------------");
	}
	...
	protected IntegralDataTypeHolder getNextValue(SessionImplementor session) {
		try {
			...
			PreparedStatement st = session.getTransactionCoordinator().getJdbcCoordinator().getStatementPreparer()
					.prepareStatement(sql);
			try {
				ResultSet rs = session.getTransactionCoordinator().getJdbcCoordinator().getResultSetReturn()
						.extract(st);
				try {
					rs.next();
	...
	protected String getSequenceNextValString(String sequenceName, int incrementSize) {
		return "select setval('" + sequenceName + "', nextval('" + sequenceName + "') + " + incrementSize + ")";
	}
	...
	protected String determineSequenceName(Properties params, Dialect dialect) {
		...
		if (sequenceName.indexOf('.') < 0) {
			...
			sequenceName = Table.qualify(dialect.quote(catalogName), dialect.quote(schemaName),
