...
public class CheckPointErrorReportTest {
	...
	public void verifyJsonGeneration() throws Exception {
		...
		// Error source and value not null
		{
			...
			CheckPointErrorReport error = new CheckPointErrorReport(null, "Neptune-Checkpoint-1", "neptune_checkpoint_1", location, "test");
		...
		// Error targets and ref value not null
		{
			...
			CheckPointErrorReport error = new CheckPointErrorReport(null, "Neptune-Checkpoint-1", "neptune_checkpoint_1", null, "", "ref_test", locationTarget);
			error.print(stream, new StringBuilder(), 1, true);
			String text = oStream.toString();
			JSONObject res = new JSONObject(text);
