...
public class LineInserter implements Inserter<Line> {
	...
	private Inserter<Route> routeUpdater;
...
//		Monitor monitor = MonitorFactory.start(BEAN_NAME);
		...
		for (GroupOfLine item : addedGroupOfLine) {
			...
			if (groupOfLine == null) {
				if (groupOfLines == null) {
					groupOfLines = groupOfLineDAO.findByObjectId(InserterUtils.getObjectIds(addedGroupOfLine));
		...
		for (Route item : addedRoute) {
			...
			if (route == null) {
				if (routes == null) {
					routes = routeDAO.findByObjectId(InserterUtils.getObjectIds(addedRoute));
		...
		for (StopArea item : addedRoutingConstraint) {
			...
			if (routingConstraint == null) {
				if (routingConstraints == null) {
					routingConstraints = stopAreaDAO.findByObjectId(InserterUtils.getObjectIds(addedRoutingConstraint));
