...
@Stateless(name = StopAreaInserter.BEAN_NAME)
@Log4j
public class StopAreaInserter implements Inserter<StopArea> {
	...
	public void insert(Context context, StopArea oldValue, StopArea newValue) throws Exception {
		...
		for (AccessPoint item : addedAccessPoint) {
			...
			if (accessPoint == null) {
				if (accessPoints == null) {
					accessPoints = accessPointDAO.findByObjectId(InserterUtils.getObjectIds(addedAccessPoint));
		...
		for (AccessLink item : addedAccessLink) {
			...
			if (accessLink == null) {
				if (accessLinks == null) {
					accessLinks = accessLinkDAO.findByObjectId(InserterUtils.getObjectIds(addedAccessLink));
		...
		for (StopArea item : addedStopAreas) {
			...
			if (area == null) {
				if (stopAreas == null) {
					stopAreas = stopAreaDAO.findByObjectId(InserterUtils.getObjectIds(addedStopAreas));
