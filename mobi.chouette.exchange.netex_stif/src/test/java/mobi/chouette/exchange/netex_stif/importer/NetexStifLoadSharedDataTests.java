...
public class NetexStifLoadSharedDataTests extends Arquillian implements Constant, ReportConstant {
	...
	public static EnterpriseArchive createDeployment() {
		...
		final WebArchive testWar = ShrinkWrap.create(WebArchive.class, "test.war").addAsWebInfResource("postgres-ds.xml")
				.addClass(NetexStifLoadSharedDataTests.class)
	...
	public void verifyCheckLoadData() throws Exception {
		...
		Assert.assertEquals(referential.getSharedReadOnlyLines().size(), 1, "line size");
		for (String key : referential.getSharedReadOnlyLines().keySet()) {
			Assert.assertEquals(key,"STIF:CODIFLIGNE:Line:C00108","objectid should be good for lines");
		}
		Assert.assertEquals(referential.getSharedReadOnlyCompanies().size(), 1, "company size");
		for (String key : referential.getSharedReadOnlyCompanies().keySet()) {
			Assert.assertEquals(key,"STIF:CODIFLIGNE:Operator:011","objectid should be good for companies");
		}
		Assert.assertEquals(referential.getSharedReadOnlyStopAreas().size(), 33, "stops size");
